...
def show():
    ...
    if st.session_state.conversation_history:
        ...
        for i, turn in enumerate(st.session_state.conversation_history):
            with st.expander(
                f"Query ({i + 1}): {turn['query']}",
                expanded=(i == len(st.session_state.conversation_history) - 1),
            ):
                if "error" in turn:
    ...
    with st.spinner("Running agent query..."):
        try:
            ...
            request = CoreRequest(
                ...
                output_test=turn.get("response", "")
                if (turn := st.session_state.conversation_history)
                else "",
            )
            # Run the query
            ...
            error_msg = (
                f"Error running query: {str(e)}\n\n{traceback.format_exc()}"
            )
            st.error(error_msg)
            # Store error in history
            turn = {"query": query, "error": str(e)}
            st.session_state.conversation_history.append(turn)
...
async def process_query_async(client, request):
    ...
    async for chunk in client.process_query(
        request, streaming=True, summarize=True
    ):
