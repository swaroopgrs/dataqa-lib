...
def show():
    ...
    st.markdown(
        "*Automatically extract schema and metadata from your data sources*"
    )
    ...
    if scanner_config_path.exists():
        try:
            ...
            with st.expander("Configuration", expanded=True):
                ...
                data_files = config.get("executor_config", {}).get(
                    "data_files", []
                )
                ...
                for i, file_config in enumerate(data_files):
                    with st.expander(
                        f"{file_config.get('table_name', f'File {i + 1}')}"
                    ):
                        ...
                        with col2:
                            st.text(
                                f"Table Name: {file_config.get('table_name', '')}"
                            )
                        ...
                        with col2:
                            if st.button("Remove", key=f"remove_{i}"):
                                data_files.pop(i)
                                config["executor_config"]["data_files"] = (
                                    data_files
                                )
                                st.rerun()
                ...
                for table_name in [f.get("table_name", "") for f in data_files]:
                    if table_name:
                        cols_text_input = st.text_input(
                            ...
                            value=",".join(
                                categorical_cols.get(table_name, [])
                            ),
                        )
                ...
                with col1:
                    ...
                    config["parameters"]["column_example_value_count"] = (
                        st.number_input(
                            "Sample Values Count",
                            min_value=1,
                            max_value=20,
                ...
                with col2:
                    config["parameters"][
                        "metadata_inference_sample_row_count"
                    ] = st.number_input(
    ...
    st.info(
        "LLM credentials are required for metadata inference (generating descriptions)"
    )

    col1, col2 = st.columns(2)
    with col1:
        cert_path = st.text_input(
            "Certificate Path (optional)", value=os.environ.get("CERT_PATH", "")
        )
        if cert_path:
    ...
    for name, file_path in schema_files.items():
        if file_path.exists():
            with st.expander(f"{name}", expanded=False):
                ...
                with col1:
                    st.info(
                        f"File: {file_path} (Size: {file_path.stat().st_size} bytes, Modified: {pd.to_datetime(file_path.stat().st_mtime, unit='s')})"
                    )

                with col2:
                    if st.button("Use as Final Schema", key=f"use_{name}"):
                        copy_to_final_schema(
                            file_path, project_path / "data" / "schema.yml"
                        )
                ...
                try:
                    ...
                    if len(content) > 5000:
                        st.info(
                            f"File is large ({len(content)} characters). Showing first 5000 characters."
                        )
                    ...
                    col_info.append(
                        {
                            ...
                            "Sample Values": ", ".join(
                                [str(x) for x in df[col].dropna().unique()[:5]]
                            ),
                        }
                    )
...
def run_data_scanner_pipeline(
    config: dict, extract_only: bool, overwrite: bool
):
        ...
        try:
            ...
            if not overwrite:
                if extracted_file.exists() or inferred_file.exists():
                    st.warning(
                        "Output files already exist. Enable 'Overwrite Existing Files' to proceed."
                    )
                    return
            ...
            if not extract_only:
                ...
                if not os.environ.get("OPENAI_API_BASE"):
                    st.error(
                        "OpenAI API Base is required for metadata inference"
                    )
                    return
                try:
                    ...
                    st.info(
                        "Schema extraction was successful. You can use the extracted schema without metadata inference."
                    )
