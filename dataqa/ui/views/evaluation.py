...
def show():
    ...
    if eval_config_path.exists():
        with st.sidebar.expander("Current Configuration", expanded=True):
            try:
                ...
                with col1:
                    config["num_run"] = st.number_input(
                        "Number of Runs",
                        min_value=1,
                        max_value=10,
                    ...
                    config["run_prediction"] = st.checkbox(
                        "Run Prediction",
                        value=config.get("run_prediction", True),
                    )

                with col2:
                    config["run_llm_eval"] = st.checkbox(
                        "Run LLM Evaluation",
                        value=config.get("run_llm_eval", True),
                    )
                    config["resume"] = st.checkbox(
                        "Resume Previous Run", value=config.get("resume", False)
                    )
                    config["debug"] = st.checkbox(
                        "Debug Mode", value=config.get("debug", False)
                    )
    ...
    if eval_data_path.exists():
        try:
            ...
            active_items = [
                item for item in test_items if item.get("active", True)
            ]
            ...
            with col3:
                st.metric(
                    "Inactive Test Cases", len(test_items) - len(active_items)
                )
            ...
            if test_items:
                ...
                if selected_id:
                    selected_item = next(
                        item for item in test_items if item["id"] == selected_id
                    )
                    with st.expander(
                        f"Details for {selected_id}", expanded=True
                    ):
                        st.markdown(
                            f"**Question:** {selected_item.get('question', '')}"
                        )
                        st.markdown("**Ground Truth Output:**")
                        st.code(
                            selected_item.get("ground_truth_output", {}).get(
                                "language", "text"
                            )
                        )
                        st.markdown("**Solution:**")
                        sql = (
                            selected_item["solution"][0]
    ...
    if results_dir.exists():
        ...
        st.info(
            "No evaluation results found. Run an evaluation to see results here."
        )
    ...
    if eval_summary_path.exists():
        try:
            ...
            if "accuracy" in results:
                ...
                with col1:
                    st.metric(
                        "Micro Accuracy", f"{results['accuracy']['micro']:.2%}"
                    )
                with col2:
                    st.metric(
                        "Macro Accuracy", f"{results['accuracy']['macro']:.2%}"
                    )
                with col3:
            ...
            if "use_case" in results["accuracy"]:
                ...
                if use_case_data:
                    st.dataframe(
                        pd.DataFrame(use_case_data), use_container_width=True
                    )
...
def show_detailed_test_result(results_dir: Path, test_id: str):
    ...
    try:
        ...
        with st.expander(f"Detailed Results: {test_id}", expanded=True):
            # Input data
            st.markdown(
                f"**Question:** {result.get('input_data', {}).get('question', '')}"
            )
            st.code(
                result.get("input_data", {}).get("ground_truth_output", ""),
                language="text",
            )
            ...
            for i, pred in enumerate(predictions):
                with st.expander(f"Run {pred.get('run_id', i + 1)}"):
                    ...
                    with col1:
                        st.markdown(
                            f"**Result:** {pred.get('evaluation', {}).get('llm_label', 'Unknown')}"
                        )
                        st.markdown(
                            f"**Latency:** {pred.get('latency', 0):.2f}s"
                        )

                    with col2:
                        st.markdown("**LLM Judge Output:**")
                        st.markdown(
                            f"**Score:** {pred.get('evaluation', {}).get('llm_judge_output', {}).get('SCORE', 'N/A')}"
                        )
                        st.markdown(
                            f"**Reason:** {pred.get('evaluation', {}).get('llm_judge_output', {}).get('REASON', 'N/A')}"
                        )
