...
from dataqa.core.components.knowledge_extraction.rule_inference_batch_test import RuleInferenceExperiment

def show():
    st.header("📜 Rule Inference")
    st.markdown("*Automatically infer business rules by comparing generated and expected SQL queries*")
        ...
        if rule_config_path.exists():
            ...
            with st.expander("Configuration", expanded=True):
                ...
                with col2:
                    ...
                    config['original_config_file'] = str(project_path / "agent_config.yml")
                    config['test_data_file'] = str(project_path / "data" / "evaluation_data.yml")
                    config['output_file_path'] = str(project_path / "output" / "rule_inference")
            ...
            st.error(f"Rule inference configuration not found: {rule_config_path}")
            return
        ...
        if test_data_path.exists():
            ...
            for item in test_data.get('data', []):
                if item.get('active', True) and \
                   ...
                   item.get('component_groundtruth', {}).get('ground_truth_output'):
            ...
            with col3:
                st.metric("Invalid Cases", len(test_data.get('data', [])) - len(valid_items))
            ...
            if valid_items:
                ...
                with st.expander("Valid Test Cases"):
                    ...
                    for item in valid_items:
                        df_data.append({
                            ...
                            'Has SQL': bool(item.get('solution', [{}])[0].get('function_arguments', {}).get('sql', '')),
                            'Has Ground Truth': bool(item.get('component_groundtruth', {}).get('ground_truth_output'))
                        })
                ...
                if mode == "Specific cases":
                    ...
                    selected_ids = st.multiselect("Select test case IDs", available_ids)
                elif mode == "First N cases":
                    n_cases = st.number_input("Number of cases", min_value=1, max_value=len(valid_items), value=min(5, len(valid_items)))
                    selected_ids = [item['id'] for item in valid_items[:n_cases]]
                
                st.info(f"Selected {len(selected_ids or [])} test cases: {', '.join(selected_ids or [])}")
            else:
    ...
    with col2:
        ...
        if config.get('multi_tenant_subscription'):
            cert_path = st.text_input("Certificate Path",
    ...
    inference_mode = st.radio(
        "Inference Mode",
        ["New Questions (Individual)", "Update Rules (Batch)", "Consolidate Rules"]
    )
    
    if st.button("🚀 Start Rule Inference", disabled=not (api_key and api_base)):
    ...
    with st.spinner(f"Running rule inference ({mode})..."):
        try:
            ...
            experiment = RuleInferenceExperiment(
                ...
                multi_tenant_subscription=config.get('multi_tenant_subscription', False)
            )
            ...
            if mode == "individual":
                ...
                if hasattr(experiment, 'experiment_result') and experiment.experiment_result:
                    combined_rules = asyncio.run(experiment.consolidate_rules())
                    st.success("✅ Rule consolidation completed!")
                else:
                    st.error("No experiment results found. Run tune questions or update rules first.")
                    return
            ...
            if combined_rules:
                ...
                if st.button("Save to rules.yml"):
                    save_consolidated_rules(combined_rules, config['config_path'])
            else:
                st.error("No experiment results found. Run tune questions or update rules first.")
            st.rerun()
        except Exception as e:
...
def show_rule_inference_results(results_dir: Path):
            ...
            try:
                ...
                if 'llm_label' in df.columns:
                    ...
                    with col1:
                        ...
                        st.metric("Correct", f"{correct_count} ({correct_count/len(df):.2%})")
                    with col2:
                        wrong_count = (df['llm_label'] == 'wrong').sum()
                        st.metric("Wrong", f"{wrong_count} ({wrong_count/len(df):.2%})")
                    with col3:
                        if 'iteration_count' in df.columns:
                            avg_iterations = df['iteration_count'].mean() if len(df.columns) else 0
                ...
                if results:
                    ...
                    if len(results) > 0 and len(results[0]) > 0:
                        with st.expander("Sample Extracted Rules"):
                            ...
                            if hasattr(rules, 'rules') and hasattr(rules.rules[0], 'rules'):
    ...
    if not result_files and not pickle_files:
        st.info("No rule inference results found. Run rule inference to see results here.")
...
def save_consolidated_rules(rules_text: str, project_path: str):
    ...
    try:
        ...
        with open(rules_file_path, 'w') as f:
            yaml.dump({'rules': all_rules}, f, default_flow_style=False, sort_keys=False)
        
        st.success(f"Saved {len(rules_list)} consolidated rules to {rules_file_path}")
    except Exception as e:
