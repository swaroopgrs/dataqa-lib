...
def show():
    ...
    with col1:
        ...
        if st.session_state.projects_dir.exists():
            projects = [
                d for d in st.session_state.projects_dir.iterdir() if d.is_dir()
            ]
            ...
            if projects:
                ...
                if st.button("Load Project"):
                    st.session_state.current_project = (
                        st.session_state.projects_dir / selected_project_name
                    )
        ...
        if st.button("Create Project"):
            if new_project_name:
                create_new_project(
                    st.session_state.projects_dir, new_project_name
                )
                st.rerun()
...
def create_new_project(projects_dir: Path, project_name: str):
    ...
    try:
        ...
        create_default_rule_inference(
            project_path / "rule_inference_config.yml"
        )
        ...
        create_default_evaluation_data(
                ...
                try:
                    with open(file_path, "r") as f:
                        ...
                        if len(content) > 2000:
                            ...
                            st.info(
                                f"File is large ({len(content)} characters). Showing first 2000 characters."
                            )
                        else:
                            st.code(content, language="yaml")
                except Exception as e:
        ...
        "workers": {
            "retrieval_worker": {
                "sql_execution_config": {
                    ...
                    "data_files": [
                        {
                            "path": "CONFIG_DIR/data/my_table.csv",
                            "table_name": "my_table",
                        }
                    ],
                }
