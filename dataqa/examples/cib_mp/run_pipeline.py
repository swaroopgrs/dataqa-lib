...
async def run_pipeline(
    config_path: Path, base_dir: Path, initial_input: PipelineInput
):
    ...
    try:
        ...
        resolved_config_content = raw_config_content.replace(
    ...
    provider_def = next(
        (
            item
            for item in config_dict.get("components", [])
            if item["name"] == "gpt_4o_model_provider"
        ),
        None,
    )
    if provider_def:
        if (
            "azure_endpoint" not in provider_def["params"]
            or not provider_def["params"]["azure_endpoint"]
        ):
            provider_def["params"]["azure_endpoint"] = os.getenv(
                "AZURE_OPENAI_ENDPOINT"
            )
            logger.info(
                "Attempting to use AZURE_OPENAI_ENDPOINT environment variable."
            )
        if (
            "api_key" not in provider_def["params"]
            or not provider_def["params"]["api_key"]
        ):
            provider_def["params"]["api_key"] = os.getenv(
                "AZURE_OPENAI_API_KEY"
            )
            logger.info(
                "Attempting to use AZURE_OPENAI_API_KEY environment variable."
            )
        # Basic check if credentials are still missing
        if not provider_def["params"].get("azure_endpoint") or not provider_def[
            "params"
        ].get("api_key"):
            logger.error(
                "Azure OpenAI endpoint or API key is missing. Set in config or environment variables (AZURE_OPENAI_ENDPOINT, AZURE_OPENAI_API_KEY)."
            )
            return None
    ...
    logger.info(
        f"Initial pipeline input prepared: {initial_input.model_dump_json(indent=2)}"
    )
    ...
    try:
        final_state_dict = await compiled_graph.ainvoke(
    ...
    try:
        ...
        if final_state.final_output:
            pprint(
                final_state.final_output.model_dump(exclude_none=True), indent=2
            )
        else:
...
async def main():
    ...
    )  # Adjust if project structure is different
...
if __name__ == "__main__":
    ...
    if not config_file_path.is_file():
        ...
        logger.error(
            "Please ensure 'config/config.yaml' exists relative to this script."
        )
    else:
        ...
        expected_files = [
            base_dir_path
            / "examples/payments/data/FAKE_PROD_BD_TH_FLAT_V3.csv",
            base_dir_path
            / "examples/payments/data/FAKE_ETS_D_CUST_PORTFOLIO.csv",
        ...
        if data_files_ok:
            ...
            logger.error(
                "Please ensure all required data and prompt files exist."
            )
