...
def mock_asset_callable(
    config_id: uuid.UUID, file_types: Set[FileType]
) -> IngestionData:
    """
    ...
    """
    print(
        f" MOCK [asset_callable]: Called for config_id={config_id} with types={file_types}"
    )
    ...
    if FileType.SCHEMA in file_types:
        ...
        ingestion_data["schema"] = DatabaseSchema(
            **yaml.safe_load(open(schema_path))
        )
    if FileType.EXAMPLES in file_types:
        examples_path = DATA_DIR / "examples.yml"
        ingestion_data["examples"] = Examples(
            **yaml.safe_load(open(examples_path))
        )
...
def mock_storage_callable(data: bytes, path_suffix: str) -> str:
    ...
    print(
        f" MOCK [storage_callable]: 'Saving' {len(data)} bytes to {fake_s3_path}"
    )
...
class MockLLMService:
    ...
    def __init__(self):
        ...
        print(
            " MOCK [LLM Service]: Initializing real AzureOpenAI client for mock..."
        )
    ...
    async def llm_invoke_with_retries(
        ...
        """
        print(
            f" MOCK [llm_invoke_with_retries]: Invoking model '{model}' with {len(messages)} messages..."
        )
...
class MockSQLService:
    ...
    def get_sql_executor(self):
        if self._sql_executor is None:
            print(
                " MOCK [SQL Service]: Initializing InMemoryCodeExecutor (DuckDB) for mock..."
            )
            config = {
                "name": "mock_sql_executor",
                "data_files": [
                    {
                        "path": str(DATA_DIR / "FAKE_PROD_BD_TH_FLAT_V3.csv"),
                        "table_name": "PROD_BD_TH_FLAT_V3",
                    },
                    {
                        "path": str(DATA_DIR / "FAKE_ETS_D_CUST_PORTFOLIO.csv"),
    ...
    async def sql_callable(
        self, config_id: uuid.UUID, sql_query: str
    ) -> pd.DataFrame:
