...
for company in companies:
    for _ in range(num_subsidiaries_per_company):
        cust_extl_id = fake.unique.random_int(
            min=100, max=999
        )  # Unique identifier for each subsidiary
        mcc_cd = fake.unique.random_int(
            min=5000, max=5999
        )  # Unique MCC code for each subsidiary

        subsidiary = {
            "CUST_KEY": fake.unique.random_int(min=100000, max=999999),
            "CUST_ID": fake.unique.random_int(min=100000, max=999999),
            "BANK_ENTERPRISE_CUST_ID": fake.unique.random_int(
...
for _ in range(num_transactions):
    ...
    acct_country, settled_currency, country, currency = random.choice(
        list(countries_currencies)
    )
    gross_sales_units = np.random.choice(
        [1, 0], p=[0.9, 0.1]
    )  # 0=false, 1=true
    gross_sales_usd = (
        0.0 if gross_sales_units == 0 else float(random.randint(100, 12000))
    )
